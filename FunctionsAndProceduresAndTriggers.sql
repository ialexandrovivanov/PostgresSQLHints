use SoftUni
--FUNCTION WHICH RETURNS SINGLE VALUE
GO
...
SELECT E.FirstName, dbo.udf_ProjectDurationWeeks (E.HireDate, '2019-02-02') AS [Weeks On Work]
  FROM Employees AS E
...
DROP FUNCTION udf_ProjectDurationWeeks --DROP FUNCTION

 --FUNCTION WHICH RETURNS TABLE
GO
...
SELECT * FROM dbo.udf_AverageSalaryByDepartment('Sales')
 ...
 --FUNCTION WHICH RETURNS TABLE AS VARIABLE
 GO
...
AS
  SELECT FirstName, DATEDIFF(Year, HireDate, GETDATE()) AS [Years On Work]
  FROM Employees
...
INSERT INTO Employees		--USE OF STORED PROCEDURE TO FILL A TABLE
EXEC usp_SelectEmployeesBySeniority  --CAN BE USED WITHOUT bdo.

DROP PROC usp_SelectEmployeesBySeniority  --DROPING PROCEDURE	

EXEC sp_depends 'usp_SelectEmployeesBySeniority' --CHECK FOR DEPENDENCIES FOR A STORE PROCEDURE
EXEC sp_depends 'Employees'						 --CHECK A COLUMN FOR DEPENDENCIES ON PROCEDURES



--USER DEFINED PROCEDURE WITH PARAMETER AND DEFAULT VALUE WHEN VALUE IS NOT USED
GO
...
EXEC usp_AddCustomer        --ADD PARAMETERS BY PARAMETER NAME
  @customerID = 'ALFKI',
...
EXEC usp_AddCustomer 'ALFKI2', 'Alfreds Futterkiste', 'Obere Str. 57', 'Berlin', '030-0074321' --ADD PARAMETERS BY POSITION


CREATE PROCEDURE dbo.usp_AddNumbers    --PROCEDURE WITH OUT PARAMETER
...
BEGIN TRY   --CAN BE USED IN PROCEDURES IN ORDER TO RETURN ERROR DESCRIPTIONS IN RESULT FIELD
...
--Find all triggers with given name from sys.triggers

SELECT * FROM [sys].[triggers] WHERE [name] LIKE '%'


--Select minimum average salary from all departments
